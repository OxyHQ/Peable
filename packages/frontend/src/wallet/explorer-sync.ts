 ...
 *
 * The web build has no TCP socket (`p2p/socket-provider.ts` falls back to a
 * provider that throws), so it cannot build a UTXO set from the P2P network.
 * It has an xpub instead, and the chain is public — this walks the address tree
 * the way any wallet does on restore, asking the Explorer instead of peers.
 *
 * The result feeds the SAME `utxoSet` the SPV path feeds, so balance, history
 * and coin selection downstream are unchanged.
 */

import type { NetworkType } from '@fairco.in/core';
import { fetchAddressInfo, type AddressInfo } from '../services/explorer-address';
